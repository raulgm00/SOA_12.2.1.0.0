      ...
      <soap:faultcode>
        <xsl:choose>
          <xsl:when test="$OperacionSvc = 'getConsultaBC'">
            <xsl:value-of select="&quot;nmp:consultaBCFault&quot;"/>
          </xsl:when>
          <xsl:otherwise>
            <!--$OperacionSvc = 'getConsultaVigenteBC'-->
            <xsl:value-of select="nmp:faultConsultaVigenteBC"/>
          </xsl:otherwise>
        </xsl:choose>
      </soap:faultcode>
      <soap:faultstring>
                    ...
                    <xsl:choose>
                      <xsl:when test="starts-with(xp20:upper-case($fault/ctx:details/ctx1:ReceivedFaultDetail/ctx1:faultstring), 'EX' )">
                        <xsl:call-template name="codigoErrorSap">
                          <xsl:with-param name="ErrorSap" select="$fault/ctx:details/ctx1:ReceivedFaultDetail/ctx1:faultstring"/>
                        </xsl:call-template>
                      </xsl:when>
                      <xsl:otherwise>
