...
declare variable $respuestaBRM as element() (:: schema-element(ns1:valorRiesgoResponse) ::) external;

declare function crd:RespuestaConsultaAutenticaCredito($idCliente as xs:int,
                                                       $calificacionBuro as xs:string, 
                                                       $folioConsultaBuro as xs:string, 
                                                       $fechaConsultaBuro as xs:date, 
                                                       $origenConsultaBuro as xs:string,
                                                       $respuestaBRM as element() (:: schema-element(ns1:valorRiesgoResponse) ::)) 
                                                       as element() (:: schema-element(nmp:consultaBCResponse) ::) {
    <nmp:consultaBCResponse>
        <nmp:respuesta>{fn:data($respuestaBRM/ns1:resultado)}</nmp:respuesta>
