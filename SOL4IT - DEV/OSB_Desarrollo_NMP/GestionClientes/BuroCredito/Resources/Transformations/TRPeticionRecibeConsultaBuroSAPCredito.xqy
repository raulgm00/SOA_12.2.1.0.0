...
declare function crd:PeticionRecibeBuroCredito($idCliente as xs:long, $probabilidadIncumplimiento as xs:decimal, $siva as xs:int, $consultaBC as element() (:: schema-element(bc:consultaXMLResponse) ::), $tipoAutorizacionConsulta as xs:string) as element() (:: schema-element(soc:ZlmRfcRecibeBuro) ::) {
    <soc:ZlmRfcRecibeBuro>
        <Cliente>{fn:string(fn:data($idCliente))}</Cliente>
        <ConsultasEfectuadas>
            {
		...
		<DeclaracionesCliente>
            ...
            <Declaracion>{fn:data($consultaBC/return/Personas/Persona/DeclaracionesCliente/DeclaracionConsumidor)}</Declaracion>
		</DeclaracionesCliente>
        <Direccion>
            {
        ...
        </Encabezado>
      {
        ...
        <ErrorUR>
            ...
            <Requerido1>{fn:data($consultaBC/return/Personas/Persona/Error/UR/FaltaCampoRequerido)}</Requerido1>
		</ErrorUR>
        {
        ...
        <PI>{fn:string(fn:data($probabilidadIncumplimiento))}</PI>
        <ResumenReporte>
            {
        ...
        <SIVA>{fn:string(fn:data($siva))}</SIVA>		
        <Score>
...
crd:PeticionRecibeBuroCredito($idCliente, $probabilidadIncumplimiento, $siva, $consultaBC, $tipoAutorizacionConsulta)