...
declare variable $subproducto as xs:string external;
...
declare function loc:TransformarAplicaMatrizKoXQ($idCliente as xs:long, $probabilidadIncumplimiento as xs:decimal, $siva as xs:int,
                                                 $respuestaBuroCredito as element() (:: schema-element(src:consultaXMLResponse) ::), $tipoAutorizacionConsulta as xs:string,  $producto as xs:string, $subproducto as xs:string) as element() (:: schema-element(trg:ZlmRfcRecibeBuro) ::) {
    <trg:ZlmRfcRecibeBuro>
      ...
      <Producto>{fn:data($producto)}</Producto>
      <ResumenReporte>
      {
      ...
      <Subproducto>{fn:data($subproducto)}</Subproducto>
      <TipoAutoriza>{$tipoAutorizacionConsulta}</TipoAutoriza>
    </trg:ZlmRfcRecibeBuro>
};

loc:TransformarAplicaMatrizKoXQ($idCliente, $probabilidadIncumplimiento, $siva, $respuestaBuroCredito, $tipoAutorizacionConsulta, $producto, $subproducto)