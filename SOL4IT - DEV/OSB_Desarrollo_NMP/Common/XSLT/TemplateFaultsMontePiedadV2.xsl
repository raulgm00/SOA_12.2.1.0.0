...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="faultstring">
       ...
       <xsl:param name="serviceName"/>
        <xsl:choose>
      <xsl:when test="$errorCode=380000"><xsl:value-of select="concat('Se ha excedido el tiempo de espera del servicio ',$serviceName)"/></xsl:when>
      <xsl:when test="$errorCode=380001">Error interno del servidor</xsl:when>
      <xsl:when test="$errorCode=380002"><xsl:value-of select="concat('Error conectividad – El destino: ',$serviceName,' es inalcanzable ')"/></xsl:when>
      <xsl:when test="$errorCode=382500"><xsl:value-of select="concat('Se ha recibido un error del servicio ',$serviceName)"/></xsl:when>
      <xsl:when test="$errorCode=382501"><xsl:value-of select="concat('Se ha recibido un error no reconocido del servicio ',$serviceName)"/></xsl:when>
      <xsl:when test="$errorCode=382502"><xsl:value-of select="concat('Error conectividad – No se encuentra el servicio ',$serviceName)"/></xsl:when>
      <xsl:when test="$errorCode=382505">Error en los parámetros del mensaje</xsl:when>
      <xsl:when test="$errorCode > 380000 and $errorCode &lt; 380100">Error en tiempo de ejecución en el transporte o proxy</xsl:when>
      <xsl:when test="$errorCode > 381999 and $errorCode &lt; 382500">Error en tiempo de ejecución en el flujo de mensajes</xsl:when>
      <xsl:when test="$errorCode > 382499 and $errorCode &lt; 383000">Error en tiempo de ejecución en las acciones del flujo de mensajes</xsl:when>
      <xsl:when test="$errorCode > 385999 and $errorCode &lt; 387000">Error de seguridad</xsl:when>
      <xsl:when test="$errorCode > 394499 and $errorCode &lt; 395000">Error en el sub sistema UDDI</xsl:when>
  ...
  <xsl:template name="descripcionError">
      ...
      <xsl:when test="$errorCode=382500">
          ...
          <xsl:otherwise><xsl:value-of select="concat('Se ha recibido un error del servicio ',$serviceName)"/></xsl:otherwise>
        </xsl:choose>
            </xsl:when>
      <xsl:when test="$errorCode=382501">
          ...
          <xsl:otherwise><xsl:value-of select="concat('Se ha recibido un error no reconocido del servicio ',$serviceName)"/></xsl:otherwise>
        </xsl:choose>
            </xsl:when>
      <xsl:when test="$errorCode=382502">
          ...
          <xsl:otherwise><xsl:value-of select="concat('Error conectividad – No se encuentra el servicio ',$serviceName)"/></xsl:otherwise>
        </xsl:choose>
            </xsl:when>
