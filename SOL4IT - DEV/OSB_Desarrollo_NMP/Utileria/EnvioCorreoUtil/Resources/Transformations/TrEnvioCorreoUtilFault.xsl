<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ctx="http://www.bea.com/wli/sb/context" xmlns:ec="http://servicios.montedepiedad.com.mx/NMP/Schema/EnvioCorreoUtil" xmlns:ns1="http://servicios.montedepiedad.com.mx/NMP/Schema/NMPStandardMessage" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <xsl:variable name="codigoError">
    ...
    <xsl:variable name="faultstring">
	   ...
	   <xsl:choose>
            <xsl:when test="$app =1">
                 <xsl:text>Envio Correo Error</xsl:text>
              </xsl:when>
          </xsl:choose>
        </xsl:when>
        <xsl:otherwise>
    ...
    <!---Generate Fault -->
    <soap:Fault>
      <faultcode>ec:envioCorreoUtilFault</faultcode>
      <faultstring>
