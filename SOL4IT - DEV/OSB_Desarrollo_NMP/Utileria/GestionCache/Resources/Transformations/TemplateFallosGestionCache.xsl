...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="faultstringNMP">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$errorCode='101'">La llave no fue encontrada</xsl:when>   
            <xsl:when test="$errorCode='102'">Cache sin llaves</xsl:when>
        </xsl:choose>
    </xsl:template>
    <xsl:template name="codigoErrorNMP">
     ...
     <xsl:template name="tipoErrorNMP">
        ...
        <xsl:choose> 
            ...
            <xsl:when test="$errorCode='101'">Error de Aplicación</xsl:when>     
            <xsl:when test="$errorCode='102'">Error de Aplicación</xsl:when>
        </xsl:choose>
    </xsl:template>
