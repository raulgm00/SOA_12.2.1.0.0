<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ctx="http://www.bea.com/wli/sb/context" xmlns:con1="http://www.bea.com/wli/sb/stages/transform/config" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:NMPCitas="http://servicios.montedepiedad.com.mx/NMP/Services/NMPCitas" xmlns:ns1="http://servicios.montedepiedad.com.mx/NMP/Schema/NMPStandardMessage">
  ...
  <xsl:template match="/">
        ...
        <xsl:when test="($appCode = 'OSB' and $beaCode = 382500) or  ($appCode = 'NMP')">
					<xsl:choose>
            <xsl:when test="$app =1">
							<xsl:value-of select="concat('Formiik Error: ',$descError)"/>  
						</xsl:when>
            <xsl:otherwise>
		...
		<soap:Fault>
      <faultcode>
        <xsl:value-of select="concat('NMPCitas:',$faultCode)"/>
      </faultcode>
      <faultstring>
