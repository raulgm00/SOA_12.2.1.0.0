<xsl:stylesheet version="1.0" exclude-result-prefixes="ctx soap con1 nmpsm" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ctx="http://www.bea.com/wli/sb/context" xmlns:bpm="http://servicios.montedepiedad.com.mx/NMP/Schema/EventProcessOperation" xmlns:con1="http://www.bea.com/wli/sb/stages/transform/config" xmlns:nmpsm="http://servicios.montedepiedad.com.mx/NMP/Schema/NMPStandardMessage">
    ...
    <xsl:template match="/">
        <soap:Body>
            <soap:Fault>
                ...
                <faultcode>bpm:GetActionFault</faultcode>
                <xsl:choose>
                    ...
                    <xsl:when test="$beaCode='002'">
                        <faultstring>
                            <xsl:call-template name="faultstring">
                                <xsl:with-param name="beaCode" select="382500"/>
                            </xsl:call-template>
                        </faultstring>
                    </xsl:when>       
                    <xsl:otherwise>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$beaCode='002'">
                        <detail>
                            <nmpsm:messageError>
                                <nmpsm:codigoError>NMP-005</nmpsm:codigoError>
                                <nmpsm:descripcionError>
                                    <xsl:value-of select="ctx:fault/ctx:reason"/>
                                </nmpsm:descripcionError>
                                <nmpsm:tipoError>Técnico</nmpsm:tipoError>
