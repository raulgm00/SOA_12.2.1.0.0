<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ctx="http://www.bea.com/wli/sb/context" xmlns:con1="http://www.bea.com/wli/sb/stages/transform/config" xmlns:n0="urn:sap-com:document:sap:soap:functions:mc-style" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:co="http://servicios.montedepiedad.com.mx/NMP/Schema/NMPCredito" xmlns:ns1="http://servicios.montedepiedad.com.mx/NMP/Schema/NMPStandardMessage">
     ...
     <xsl:variable name="errorCode">
      ...
      <xsl:variable name="faultstring">
        ...
        <xsl:variable name="codigoError">
           <xsl:choose>
        <xsl:when test="//n0:ZlmfmRfcDesembolso.Exception">
             <xsl:text>NMP-CRE-1001</xsl:text>
        </xsl:when>
         <xsl:when test="//n0:ZlmRfcCreaContrato.Exception">
             <xsl:text>NMP-CRE-2001</xsl:text>
        </xsl:when>
		...
		<xsl:choose>
       <xsl:when test="//n0:ZlmfmRfcDesembolso.Exception">
             <xsl:value-of select="concat(//n0:ZlmfmRfcDesembolso.Exception/Name,'-',//n0:ZlmfmRfcDesembolso.Exception/Text)"/>
        </xsl:when>
         <xsl:when test="//n0:ZlmRfcCreaContrato.Exception">
             <xsl:value-of select="concat(//n0:ZlmRfcCreaContrato.Exception/Name,'-',//n0:ZlmRfcCreaContrato.Exception/Text)"/>
        </xsl:when>
        <xsl:when test="$appCode = 'NMP'">                                    
