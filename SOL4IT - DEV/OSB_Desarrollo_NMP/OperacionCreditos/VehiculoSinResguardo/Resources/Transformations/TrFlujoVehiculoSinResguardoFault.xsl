<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ctx="http://www.bea.com/wli/sb/context" xmlns:con1="http://www.bea.com/wli/sb/stages/transform/config" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:fu="http://servicios.montedepiedad.com.mx/NMP/Services/VehiculoSinResguardo/Flujo" xmlns:ns1="http://servicios.montedepiedad.com.mx/NMP/Schema/NMPStandardMessage">
    ...
    <xsl:variable name="codigoError">
		...
		<xsl:variable name="faultstring">
			<xsl:choose>
                <xsl:when test="($appCode = 'OSB' and ($errorCode = 382500 or $errorCode = 380001 ) ) or  ($appCode = 'NMP')">
					<xsl:choose>
                        <xsl:when test="$app =1">
							<xsl:text>Flujo vehículo sin resguardo</xsl:text>
						</xsl:when>
                                                 <xsl:when test="$app =2">
							<xsl:text>Incio flujo vehículo sin resguardo</xsl:text>
						</xsl:when>
                    </xsl:choose>
				</xsl:when>
                <xsl:otherwise>
		...
		<xsl:variable name="descripcionError">
			<xsl:choose>
                         <xsl:when test="$app = 2">
							<xsl:text>No se encuentra correlación de la notificación</xsl:text>
						</xsl:when>
                ...
                <!---Generate Fault -->
		<soap:Fault>
            <faultcode>fu:flujoVehiculoFault</faultcode>
            <faultstring>
