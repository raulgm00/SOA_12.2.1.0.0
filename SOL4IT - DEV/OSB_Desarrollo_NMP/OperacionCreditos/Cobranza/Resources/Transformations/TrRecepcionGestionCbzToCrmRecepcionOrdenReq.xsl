...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">        
        <tem:RecepcionOrdenes>           
            <tem:Orden>
                <xsl:if test="ges:recepcionGestionRequest/ges:Cobranza/ges:resultadoGestion/ges:accion">
                    <nmp:CategoriaInteraccion>
                        <xsl:value-of select="ges:recepcionGestionRequest/ges:Cobranza/ges:resultadoGestion/ges:accion"/>
                    </nmp:CategoriaInteraccion>
                </xsl:if>
                <xsl:if test="ges:recepcionGestionRequest/ges:Cobranza/ges:resultadoGestion/ges:comentarios">
                    <nmp:Comentarios>
                        <xsl:value-of select="ges:recepcionGestionRequest/ges:Cobranza/ges:resultadoGestion/ges:comentarios"/>
                    </nmp:Comentarios>
                </xsl:if>                               
                <nmp:Contrato>
                ...
                <!--nmp:Descripcion/-->
                <xsl:if test="ges:recepcionGestionRequest/@fechaInicial">
                    <nmp:FechaGestion>
                        <xsl:value-of select="ges:recepcionGestionRequest/@fechaInicial"/>
                    </nmp:FechaGestion>
                </xsl:if>                               
                <nmp:GestorCobranza>
                ...
                <nmp:Producto>
                    <xsl:value-of select="ges:recepcionGestionRequest/@idProducto"/>
                </nmp:Producto>
