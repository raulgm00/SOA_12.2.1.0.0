...
declare function local:TransformarConsultaAutografaXQ($consultaAutografa as element() (:: schema-element(src:consultaAutografaBCRequest) ::)) as element() (:: schema-element(trg:ZLM_RFC_CONSULTABURO) ::) {
    <trg:ZLM_RFC_CONSULTABURO>
        <I_BURO_INFO>
            ...
            <SIVA>{fn:data($consultaAutografa/src:InputFields/src:BP_SIVA)}</SIVA>
            <PI>{fn:data($consultaAutografa/src:InputFields/src:BP_PROBINCUMPL)}</PI>
        </I_BURO_INFO>
		<I_FRECUENCIA>{fn:data($consultaAutografa/src:InputFields/src:Frecuencia)}</I_FRECUENCIA>
    </trg:ZLM_RFC_CONSULTABURO>
};
