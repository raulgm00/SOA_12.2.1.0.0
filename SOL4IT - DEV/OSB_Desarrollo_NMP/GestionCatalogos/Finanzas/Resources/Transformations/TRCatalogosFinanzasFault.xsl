<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ctx="http://www.bea.com/wli/sb/context" xmlns:con1="http://www.bea.com/wli/sb/stages/transform/config" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ca="http://servicios.montedepiedad.com.mx/NMP/Services/Catalogos/Finanzas" xmlns:ns1="http://servicios.montedepiedad.com.mx/NMP/Schema/NMPStandardMessage">
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="codigoError">
        ...
        <xsl:when test="($appCode = 'OSB' and ($errorCode = 382500 or $errorCode = 380001 ) ) or  ($appCode = 'NMP')">
					<xsl:choose>
            <xsl:when test="$app =1">
							<xsl:text>Catalogo Finanzas Error</xsl:text>
						</xsl:when>
          </xsl:choose>
				</xsl:when>
        <xsl:otherwise>
        ...
        <xsl:otherwise>
                ...
                <!---Generate Fault -->
		<soap:Fault>
      <faultcode>ca:catalogosFinanzasFault</faultcode>
      <faultstring>
