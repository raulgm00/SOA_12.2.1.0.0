<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ctx="http://www.bea.com/wli/sb/context" xmlns:ca="http://servicios.montedepiedad.com.mx/NMP/Services/NMPCatalogosSepomex" xmlns:ns1="http://servicios.montedepiedad.com.mx/NMP/Schema/NMPStandardMessage" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <xsl:variable name="codigoError">
    ...
    <xsl:variable name="faultstring">
	   ...
	   <xsl:choose>
            <xsl:when test="$app =1">
                 <xsl:text>Catalogo Sepomex Error</xsl:text>
              </xsl:when>
          </xsl:choose>
        </xsl:when>
        <xsl:otherwise>
    ...
    <!---Generate Fault -->
    <soap:Fault>
      <faultcode>ca:catalogoSepomexFault</faultcode>
      <faultstring>
